...
vadd::vadd(sc_module_name name, xsc::common_cpp::properties& _properties) : xsc::xtlm_ap_ctrl_none(name) {
	unsigned int stream_width = 32;//_properties.getLongLong("<SOME PARAM NAME>");
...
void vadd::send_response()
{
	ipc2axis_socket->send_response();
}

void vadd::axis2ipc_send() 
{
...
std::string vadd::get_ipi_name(std::string s){
